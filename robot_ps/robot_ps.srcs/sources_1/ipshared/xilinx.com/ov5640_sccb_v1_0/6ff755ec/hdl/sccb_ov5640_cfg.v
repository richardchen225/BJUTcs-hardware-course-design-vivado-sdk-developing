...
#(
parameter REG_NUM           = 248                   , //寄存器个数
parameter CMOS_H_PIXEL      = 24'd512               , //CMOS水平方向像素个数
parameter CMOS_V_PIXEL      = 24'd512               , //CMOS垂直方向像素个数
...
input   wire                sccb_vld              	, //SCCB配置有效信号 （从power_ctrl的power_done输入）
input   wire                sccb_done               , //SCCB寄存器配置完成信号 （从sccb_wr配置完一个的标志信号输入）
output  reg                 sccb_en                 , //SCCB触发执行信号  （输出到sccb_wr告知开始传输数据）
output  reg     [23:0]      sccb_data               , //SCCB要配置的地址与数据(高16位地址,低8位数据)（输出到sccb_wr）
output  reg                 sccb_cfg_done             //SCCB全部寄存器配置完成信号 （整体结束信号）
);
